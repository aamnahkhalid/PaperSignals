...
void PaperSignals::UmbrellaExecution(String JSONData)
{
  ...
  DynamicJsonBuffer weatherLocationBuffer;
  JsonObject& locationRoot = weatherLocationBuffer.parseObject(JSONData);
  //cannot print locationRoot but would be great if we could check what location root is
  String City = locationRoot["parameters"]["location"]["city"];
  String State = locationRoot["parameters"]["location"]["admin-area"];
  String Address = locationRoot["parameters"]["location"]["street-address"];
  String MaybeAlsoACity = locationRoot["parameters"]["location"]["subadmin-area"];
  ...
  Serial.println("Location printed--------------------------------");

  String WeatherJSON = GetWeather(Location);
  Serial.print("weatherJson string:");
  ...
  String iconWind = weatherRoot["daily"]["data"][0]["windSpeed"];
  Serial.println("WIND SPEEED AHEAD :::::::::::::::::::::::::::::::::::::::::::::  ");
  Serial.println(iconWind);
  ...
  {
    Serial.println("It rained so Wally moved----------*****************************************");
    MoveServoToPosition(UMBRELLA_OPEN, 10);
  }
  if (iconWeather== snowIcon){
    Serial.println("It S N O W E D ----------*****************************************");
    MoveServoToPosition(UMBRELLA_SNOW, 10);
  ...
  if ( (iconWind).toInt() >= 5){
    ...
    for (int i=0; i<50; i++){  
    MoveServoToPosition(170, 10);
    MoveServoToPosition(70, 10);
    }
    
  }
  else
  {
    Serial.println("It ain't rainin **********************************************************************************");
